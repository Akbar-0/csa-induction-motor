...
\author{\IEEEauthorblockN{Akbar Shah Zar, Abdul Sammi  Khan}\IEEEauthorblockA{
School of Electrical Engineering and Computer Science (SEECS),\\
National University of Sciences and Technology (NUST), Pakistan\\
Emails: \{amughal.bee23seecs, akhan.bee23seecs\}@seecs.edu.pk}}
...
We report macro-averaged precision, recall, and F1 across classes and visualize per-class metrics for interpretability. Figure~\ref{fig:eval} summarizes evaluation results; it complements the confusion matrix in Figure~\ref{fig:cm} and the training curves in Figure~\ref{fig:curve}.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{confusion_matrix.png}
  \caption{Evaluation metrics per class (precision, recall) and macro averages.}
  \label{fig:eval}
\end{figure}
...
The repository provides MATLAB code to reproduce results. The full codebase is available at \url{https://github.com/Akbar-0/csa-induction-motor.git}. A minimal script:
